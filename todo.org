...
** TODO [#A] Rewrite data fetching to use pathofexile.com/trade instead of poe.trade
*** TODO Split out data fetching into separate modules. One module for each backend
*** TODO Introduce some static interface for fetching offers in the specified format
*** TODO Make sure poe.trade still works just in case. Also add backend-specific tests

import unittest
from src.currency import load_items, build_item_list_poetrade


class CurrencyTest(unittest.TestCase):

    def test_currency_loading(self):
    ...
    def test_build_currency_list(self):
        data = [
            {
                "id": 4,
                "name": "Chaos Orb",
        ...
        poetrade_pairs = build_item_list_poetrade(
            data, {"use_bulk_items": True})
        poetrade_pairs = list(sorted(
            map(lambda x: (x[0]["name"], x[1]["name"]), poetrade_pairs)))
        expected_result = sorted([('Chaos Orb', 'Exalted Orb'),
